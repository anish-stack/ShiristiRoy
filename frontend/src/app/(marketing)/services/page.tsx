...
export const metadata: Metadata = buildMetadata({
  ...
  description: 'Individual counselling, family therapy, online sessions, Adlerian-integrative therapy, emotional regulation, and young adult support with Srishti Roy.',
  path: '/services',
});
...
export default async function ServicesPage() {
  let services:any = [];
  try { services = await serviceApi.list(); } catch {}
    ...
    <>
      <section className="pt-32 pb-16 px-4 bg-gradient-to-b from-[#EDE8F8]/40 to-brand-ivory">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h1 className="font-serif text-5xl sm:text-6xl text-brand-ink mb-6">Therapy services</h1>
          <p className="text-lg text-brand-ink/60 leading-relaxed">
            Each service is tailored to meet you where you are, creating space for understanding, healing, and growth.
          </p>
        </div>
      </section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {(services.length > 0 ? services : [
              ...
              <div key={s._id} className="card-soft group hover:shadow-md hover:border-brand-lavender/25 transition-all duration-300 flex flex-col">
                ...
                <p className="text-sm text-brand-ink/65 leading-relaxed flex-1">{s.shortDesc}</p>

                <div className="mt-6 pt-5 border-t border-brand-lavender/10 flex items-center justify-between">
                  <div>
                    {s.price?.amount > 0 && (
                      <p className="text-brand-lavender font-medium text-sm">₹{s.price.amount.toLocaleString('en-IN')}</p>
                    )}
                    <p className="text-xs text-brand-ink/40">{s.durationMin} min · {s.modes?.join(' & ')}</p>
                  </div>
                  <Link href={`/services/${s.slug}`} className="text-brand-lavender hover:underline text-sm font-medium flex items-center gap-1">
      ...
      <section className="section bg-[#EDE8F8]/30">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="font-serif text-4xl text-brand-ink mb-4">Not sure where to start?</h2>
          <p className="text-brand-ink/60 mb-8">Reach out and we can discuss which service best fits your needs.</p>
          <div className="flex gap-4 justify-center flex-wrap">
            <Link href="/book" className="btn-primary">Book a session <ArrowRight size={16} /></Link>
